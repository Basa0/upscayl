...
export class SettingsService {
  readonly customModelsPath = signal<string | null>(readStorage('customModelsPath', null));
  readonly selectedModelId = signal(readStorage('selectedModelId', 'upscayl-standard-4x'));
  readonly doubleUpscayl = signal(readStorage('doubleUpscayl', false));
  readonly gpuId = signal(readStorage('gpuId', ''));
  readonly saveImageAs = signal<'png' | 'jpg' | 'webp'>(readStorage('saveImageAs', 'png'));
  readonly scale = signal(readStorage('scale', '4'));
  readonly savedOutputPath = signal<string | null>(readStorage('savedOutputPath', null));
  readonly rememberOutputFolder = signal(readStorage('rememberOutputFolder', false));
  readonly compression = signal(readStorage('compression', 0));
  readonly overwrite = signal(readStorage('overwrite', false));
  readonly turnOffNotifications = signal(readStorage('turnOffNotifications', false));
  readonly ttaMode = signal(readStorage('ttaMode', false));
  readonly viewType = signal<'slider' | 'lens'>(readStorage('viewType', 'slider'));
