import { Component, computed, input, signal, HostListener, ElementRef, inject } from '@angular/core';

@Component({
  selector: 'app-comparison-slider',
  standalone: true,
  template: `
    <div class="slider" #root (pointerdown)="onDown($event)" (pointermove)="onMove($event)">
...
export class ComparisonSliderComponent {
  ...
  readonly beforeClip = computed(() => `inset(0 ${100 - this.position()}% 0 0)`);
  ...
  private update(event: PointerEvent): void {
    const rect = this.el.nativeElement.querySelector('.slider')!.getBoundingClientRect();
